(function() {
    ...
    function RightSidenavController($scope, $http, $mdSidenav, $state, API_CONFIG) {
        ...
        $http.get(API_CONFIG.url+'notifications/get')
            .success(function (response) {

                vm.notificationGroups = response;
        ...
        $http({
            method: 'GET',
            url: API_CONFIG.url +'Companies'
        }).success(function(response) {
            var data = JSON.parse(localStorage.cplan);
            data.companies.groups = response;
            data.companies.default_group_index = 0;
            data.companies.default_company_index = 0;
            //console.log(localStorage);
            localStorage.cplan = JSON.stringify(data);
        });
